...
(function ($) {
  ...
  function CLBkFuncForSpt(e) {
    setTimeout(() => { 
        let itmmain = document.querySelector('.spt_item')
        if (itmmain) {
          let itm = e.target.querySelector('.owl-item.active')
          let attr = itm.querySelector('a img')  
          itmmain.style.backgroundImage = "url("+attr.src+")" 
          attr.addEventListener('clcik', (e) => {
            e.preventDefault();
          })
        } 
    },100)
  ...
  window.addEventListener('load', () => { 
    let slderFirst = document.querySelector(".hero_slider_btn li a");
    if (slderFirst) {
      slderFirst.classList.add('active')
    }
  })
